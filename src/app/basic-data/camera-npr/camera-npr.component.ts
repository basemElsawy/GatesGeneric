...
import { CamiraNpr } from 'src/app/Interfaces/camira-npr';
...
@Component({
  selector: 'app-camera-npr',
  templateUrl: './camera-npr.component.html',
  styleUrls: ['./camera-npr.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CameraNPRComponent implements OnInit {
  isNewMode = false;
  isEditMode = false;
  isDeleteMode = false;
  page: number = 1;
  ...
  holdOriginalData: Subject<CamiraNpr[]> = new Subject<CamiraNpr[]>();
  holdListOfFilterData: Subject<CamiraNpr[]> = new Subject<CamiraNpr[]>();
  holdSingleRaw!: CamiraNpr;
  ...
  editModel(singleCamira: any) {
    this.holdSingleRaw = singleCamira as CamiraNpr;
  ...
  sendDataToServer() {
    ...
    this.apiService.PostMethodWithPipe('camira/camira_npr/', data).subscribe({
      next: (res: any) => {
  ...
  editDataInServer() {
    ...
    this.apiService
      .UpdateMethodWithPipe('camira/camira_npr/', null, sendData)
      .subscribe({
        next: (res: any) => {
          this.camiraForm.reset();
          this.updateApi.emit(res);
  ...
  removeDataFormServer() {
    this.apiService
      .DeleteMethodWithPipe('camira/camira_npr/', null, {
        camira_npr_id: this.holdSingleRaw.camira_npr_id,
      })
      .subscribe({
        next: (res: any) => {
          this.camiraForm.reset();
